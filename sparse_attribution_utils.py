...
def apply_saes_and_run(
    ...
    # Convert model activations and SAE activations to sparse tensors
    sparse_model_activations = {k: act.to_sparse() for k, act in model_activations.items()}
    sparse_sae_activations = {
        k: SaeReconstructionCache(
            sae_in=cache.sae_in.to_sparse(),
            feature_acts=cache.feature_acts.to_sparse(),
            sae_out=cache.sae_out.to_sparse(),
            sae_error=cache.sae_error.to_sparse()
        )
        for k, cache in sae_activations.items()
    }

    return ApplySaesAndRunOutput(
        model_output=model_output,
        model_activations=sparse_model_activations,
        sae_activations=sparse_sae_activations
    )
